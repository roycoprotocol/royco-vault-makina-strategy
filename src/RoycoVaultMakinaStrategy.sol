...
contract RoycoVaultMakinaStrategy is AccessManaged, Pausable, IStrategyTemplate {
    ...
    /// @dev Thrown when the Royco vault and Makina machine have disparate base assets
    error DISPARATE_VAULT_AND_MACHINE_ASSETS();
    ...
    /// @dev Thrown when the token being rescued is the machine share token
    ...
    function rescueToken(address _token, uint256 _amount) external override(IStrategyTemplate) restricted {
        // Ensure that the token to rescue is not the machine share token (used to value the strategy's position)
        require(_token != MACHINE_SHARE_TOKEN, INVALID_TOKEN_TO_RESCUE());
